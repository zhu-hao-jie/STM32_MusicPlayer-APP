...
{	 
	...
	//这里只针对SPI口初始化,  SPI_CR1 SPI控制寄存器1设置
	...
	tempreg|=1<<6;			//SPI启动 
	SPI5->CR1=tempreg; 		//设置CR1  ,SPI_CR1寄存器
...
{
	//	SPI_SR :SPI 状态寄存器
	while((SPI2->SR&1<<1)==0);		//等待发送区空 。bit 1 :发送缓冲区配置，为 1 时发送缓冲区为空
