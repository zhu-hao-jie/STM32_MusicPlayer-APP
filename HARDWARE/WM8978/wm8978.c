...
u8 WM8978_Init(void)
{
	u8 res; 
 	...
 	GPIO_Set(GPIOE,PIN2|PIN3|PIN4|PIN5|PIN6,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);	//PE2~6 复用功能输出
	...
	//R10 DAC control
	WM8978_Write_Reg(10,1<<3);	//R10,SOFTMUTE（软静音）关闭,128x采样,最佳SNR 
...
{
	u16 regval;
	regval=WM8978_Read_Reg(3);	//读取R3，电源管理控制器，
...
void WM8978_Input_Cfg(u8 micen,u8 lineinen,u8 auxen)
{
	...
	regval=WM8978_Read_Reg(44);	//读取R44  ，PGA引脚控制寄存器
...
{
	gain&=0X3F;//111111 :35.25db
	WM8978_Write_Reg(45,gain);		//R45,左通道输入PGA寄存器设置 
	//-12 db
...
void WM8978_LINEIN_Gain(u8 gain)
{
	u16 regval;
	gain&=0X07;
	regval=WM8978_Read_Reg(47);	//读取R47 ，左输入升压控制器，bit 8： 为0时具有 0db增益，为1时具有 +20db增益
	regval&=~(7<<4);			//清除原来的设置 
 	WM8978_Write_Reg(47,regval|gain<<4);//设置R47
	regval=WM8978_Read_Reg(48);	//读取R48，右输入升压控制器
	regval&=~(7<<4);			//清除原来的设置 
