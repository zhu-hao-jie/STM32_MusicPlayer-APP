...
vu8 dirmode=0;		//横竖屏模式:0,竖屏模式;1,横屏模式.
//播放mjpeg文件
//返回值:
//0,正常播放结束,返回文件浏览状态.
...
{    
	...
	{ 
		...
		if(res==0)
		{
			...
			{
				if(avix.Height<=lcddev.width&&avix.Width<=lcddev.height)//横屏时,可以显示出来,则启用横屏模式
				{
					dirmode=1;			//横屏模式
			...
			if(dirmode==0)video_info_upd(&videodev,videoprgb,volprgb,&avix,1);	//更新所有信息		
			tcnt=0;
			playflag=0;
			...
			{		
				...
				}else//横屏模式,检测按键
				{
					...
					switch(key)
					{
						...
						case KEY2_PRES:
							SAI_Play_Stop();//关闭音频
							video_seek_key(videodev.file,&avix,framebuf);
							pbuf=framebuf;
							SAI_Play_Start();//开启DMA播放 
							break;
				...
				if(system_task_return){playflag=0XFF;break;};		//TPAD返回，返回目录
			...
			}else if(playflag==0XFF){res=0;break;}
			else if(playflag==4){res=0XFF;break;}
		}
	}  
...
//avi文件查找,通过按键的方式
u8 video_seek_key(FIL *favi,AVI_INFO *aviinfo,u8 *mbuf)
{
	u32 fpos=favi->fptr;
	u8 *pbuf;
	u16 offset;
