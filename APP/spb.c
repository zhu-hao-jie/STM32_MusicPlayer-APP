...
u8 spb_init(void)
{
	...
	sramlcdbuf=gui_memex_malloc((u32)spbdev.spbheight*spbdev.spbwidth*7);	//申请3.5帧缓存大小
...
{ 
	...
	if(clr)
	{
		...
		gui_show_string("CPU:  %",+20+2+20+2,(spbdev.stabarheight-16)/2,64,16,16,WHITE);//显示CPU数据
...
//	}else gui_show_string(spb_gsma_msg[gui_phy.language][0],2+20+2,(spbdev.stabarheight-16)/2,64,16,16,WHITE);//显示运营商名字	
	//更新SD卡信息 
	if(gui_phy.memdevflag&(1<<0))app_show_mono_icos(+2,(spbdev.stabarheight-20)/2,20,20,(u8*)SPB_SD_ICO,WHITE,BLACK); //64+24
	else gui_fill_rectangle(+2,0,20,spbdev.stabarheight,BLACK);
//	//更新U盘信息 
...
//	else gui_fill_rectangle(24+64+2+20,0,20,spbdev.stabarheight,BLACK);
	...
	gui_show_num(+2+20+20+2+32,(spbdev.stabarheight-16)/2,2,WHITE,16,temp,0);//显示CPU使用率
	//显示时间
