...
//内存池(MEMx_BLOCK_SIZE字节对齐)
__align(MEM1_BLOCK_SIZE) u8 mem1base[MEM1_MAX_SIZE];													//内部SRAM内存池
__align(MEM2_BLOCK_SIZE) u8 mem2base[MEM2_MAX_SIZE] __attribute__((at(0XC01F4000)));					//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)
__align(MEM3_BLOCK_SIZE) u8 mem3base[MEM3_MAX_SIZE] __attribute__((at(0X10000000)));					//内部CCM内存池
//内存管理表
