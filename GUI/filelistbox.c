...
//磁盘名字表
u8*const FLIST_DISK_NAME_TBL[_VOLUMES][GUI_LANGUAGE_NUM]=
{
	{"SD卡","SD卡","SD Card",},
...
vu8 FLIST_DISK_ORDER_TBL[_VOLUMES];
//filelist磁盘路径表
u8*const FLIST_DISK_PATH_TBL[_VOLUMES]={"0:","1:","2:","3:"};
...
u8 filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
 	...
 	if(filelistbox->path)	//路径有效
	{
		...
		if(res==FR_OK)		//打开目录成功了.
		{
			...
			filelistbox->findextbl=(u32*)gui_memin_malloc(FLBOX_MAX_DFFILE*4);//先默认申请FLBOX_MAX_DFFILE个目标文件索引内存
 		} 
		while(res==FR_OK)
		{
			...
			{
				type=f_typetell((u8*)(flboxfinfo->fname));//获得类型
			...
			if(filelistbox_addlist(filelistbox,(u8*)flboxfinfo->fname,type))break;//添加失败了         
		}
		if(mark==1&&filelistbox->filecnt)
		{
			fn=gui_memin_realloc(filelistbox->findextbl,filelistbox->filecnt*4);//释放多余的内存
			if(fn!=NULL)filelistbox->findextbl=(u32*)fn;//如果申请成功了,则使用新的地址,否则源地址不变
		}   
...
void filelistbox_add_disk(_filelistbox_obj *filelistbox)
{
	...
	if(filelistbox->memdevflag&(1<<3)) 	//U盘在位
	{
		...
		{
			filelistbox_addlist(filelistbox,(u8*)FLIST_DISK_NAME_TBL[3][gui_phy.language],0);//添加一个磁盘
 		}  
	}		 
}
